...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[0]) return message.reply("Choose how many messages you want to delete <1-100>!");
    try {
      if (!isNaN(args[0]) && args[0] < 101 && !args[0].includes(".") && !args[0].includes(".")){
        const fetched = await message.channel.fetchMessages({limit: args[0]});
        message.channel.bulkDelete(fetched).catch(error => message.reply(`Error: ${error}`));
